...
搜索引擎的核心在于根据**关键词（term）**找到相关**文档（document）**的能力。为了实现这一目的，lucene针对每个term维护了一个倒排链（posting list），即包含该term的docId列表。
...
倒排索引是**关键词（term）**到**文档id列表**的映射关系。例如针对上面这张表格，可以建立两个倒排索引，分别是name和age Index。
...
![image](https://alidocs.oss-accelerate.aliyuncs.com/res/wYdgOkBZMdNgq4BX/img/5df5c7d4-ace5-412a-adda-800c7b37e0c5.png)

TRIE有重复的3个final state:3、8、11。 而8、11都是s转移，是可以合并的，FSA共享后缀后：

![image](https://alidocs.oss-accelerate.aliyuncs.com/res/wYdgOkBZMdNgq4BX/img/775b9484-4349-4aa1-8afa-199c383e6bd0.png)
...
![image](https://alidocs.oss-accelerate.aliyuncs.com/res/wYdgOkBZMdNgq4BX/img/d7145cfa-6550-47b6-9644-616d2a0e9e59.png)
...
3.  SkipList的层次，这个是指整个SkipList有几层![image](https://alidocs.oss-accelerate.aliyuncs.com/res/wYdgOkBZMdNgq4BX/img/0b3fdfc3-b7e4-466f-a101-cc3012d3a883.png)
...
![image](https://alidocs.oss-accelerate.aliyuncs.com/res/wYdgOkBZMdNgq4BX/img/13ddd931-b707-4653-8125-f17d100ad674.png)
...
这两种合并使用索引的方式都有其用途。Elasticsearch 对其性能有详细的[对比](https://www.elastic.co/blog/frame-of-reference-and-roaring-bitmaps?spm=ata.21736010.0.0.6f457536KcsAWX)。简单来说对于简单的相等条件的过滤缓存成纯内存的 bitset 还不如需要访问磁盘的 skip list 的方式要快。
